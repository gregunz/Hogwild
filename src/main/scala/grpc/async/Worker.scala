...
import model.{WeightsHandler, GrpcServer, SVM, SparseNumVector}
...
object Worker extends GrpcServer {
  ...
  def establishCommunications(WorkerID : Int): Unit = {
    ...
    if (WorkerID == 0){
      ...
      val addresses = blockingStub.ready(WorkerAddress(localIpAddress, response.port))
      ...
      while(working){
        ...
        val weightsUpdate = svm.updateWeight(newGradient)
  ...
  object WorkerService extends WorkerServiceGrpc.WorkerService {
    ...
    var current_workers_addresses = List[WorkerAddress]()
    ...
    override def ready(request: WorkerAddress): Future[WorkersDetails] = {
      ...
      Future.successful(WorkersDetails(current_workers_addresses))
    }
  }
